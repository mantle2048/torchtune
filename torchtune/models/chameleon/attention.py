...
class ChameleonCausalSelfAttention(CausalSelfAttention):
    def __init__(
        self,
        embed_dim: int,
        num_heads: int,
        ...
        if qk_normalization:
            self.q_normalization = nn.LayerNorm(head_dim)
            self.k_normalization = nn.LayerNorm(head_dim)
    ...
    def forward(
        ...
        *,
        ...
        if self.qk_normalization:
            ...
            q = self.q_normalization(q)
            q = q.view(-1, self.num_heads * self.head_dim)

            k = k.view(-1, self.num_kv_heads, self.head_dim)
            k = self.k_normalization(k)
            k = k.view(-1, self.num_kv_heads * self.head_dim)
        ...
        # chameleon applies the RoPE embeddings on tensors with shape
        # [b, s, n_h, h_d]
