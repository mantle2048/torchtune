...
class VocabInfo:
    def __init__(self, vocab_map: dict[str, int]):
        ...
        self.bos_id = vocab_map.get("<s>", 0)
        self.eos_id = vocab_map.get("</s>", 2)
        self.boi_id = vocab_map.get("<racm3:break>", 8197)
        self.eoi_id = vocab_map.get("<eoss>", 8196)
        self.pad_id = vocab_map.get("<pad>", 1)
        self.eot_id = vocab_map.get("<reserved08706>", 8710)
...
class VocabTranslation:
    ...
    @cached_property
    def img2bpe_mapping_tensor(self) -> torch.Tensor:
        mapping = torch.zeros(
