...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def _setup_data(
        ...
        if isinstance(cfg_dataset, ListConfig):
            datasets = [
                config.instantiate(single_cfg_dataset, tokenizer=self._token_manager)
                for single_cfg_dataset in cfg_dataset
            ]
            ds = ConcatDataset(datasets=datasets)
            packed = False
        else:
            ds = config.instantiate(cfg_dataset, tokenizer=self._token_manager)
        ...
        dataloader = DataLoader(
            ...
            collate_fn=partial(
                ...
                padding_idx=self._token_manager.vocab.pad_id,
